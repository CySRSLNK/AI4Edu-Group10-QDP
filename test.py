...
def test():
    ...
    test_dataset = dh.QuestionDataset(test_data, device, args.task_type)
    ...
    with torch.no_grad():
        for batch_cnt, batch in zip(batches, test_loader):
            ...
            if args.task_type == 'regression':
                ...
                batch_preds = scores.squeeze().cpu().numpy()
            else:
    ...
    if args.task_type == 'regression':
        ...
        test_pcc, test_doa = dh.regression_eval(true_labels, predicted_scores)
        
        logger.info("Test Results:")
        logger.info(f"Loss: {test_loss:.4f} | PCC: {test_pcc:.4f} | DOA: {test_doa:.4f} | "
                   f"RMSE: {test_rmse:.4f} | R2: {test_r2:.4f}")
        
        # Print some examples
        logger.info("\nSample Predictions (first 10):")
        for i in range(min(10, len(true_labels))):
            logger.info(f"ID: {question_ids[i]}, True: {true_labels[i]:.4f}, "
                       f"Pred: {predicted_scores[i]:.4f}, Diff: {abs(true_labels[i] - predicted_scores[i]):.4f}")
    ...
    for i in range(len(true_labels)):
        ...
        #     'original_difficulty': original_difficulties[i],
        #     'true_label': float(true_labels[i]),
        #     'predicted_score': float(predicted_scores[i]) if args.task_type == 'regression' else int(predicted_labels[i]),
        #     'text_length': test_data['text_lengths'][i]
        # }
        result = {
